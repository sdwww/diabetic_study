...
def get_config():
    ...
    model_config['n_pretrain_epoch'] = 50
...
if __name__ == '__main__':
    ...
    for i in simulated_data:
        idx = np.random.choice(5000, rate * 100, replace=False)
        rate += 1
        i[idx] = 1
    simulated_data = np.transpose(simulated_data)
    # np.savetxt('data.txt', simulated_data)
